"""
Processes module for multiview optimal pose skeleton-based action recognition.

This module contains all the core processing functions for dataset handling,
parameter management, and the main processing pipeline.
"""
...
# NTU RGB+D dataset configuration
...
def get_params(params_d):
    """Extract and return parameter values from parameter dictionary."""
...
def check_file_exist(type_d, type_params, data_preprocess, classes=None):
    """Check if processed data file exists and return file information."""
...
def get_dataset_pku(type_d='pku_cv_train', data_preprocess = False, return_gen=False, params_d = {}):
    """Load and prepare PKU-MMD dataset."""
...
def get_dataset_ntu(type_d='ntu_cs_train', data_preprocess = False, return_gen=False, params_d = {}):
    """Load and prepare NTU RGB+D dataset."""
    classes = params_d['classes']
    kalman_params = params_d['kalman_f']
...
def prepare_dataset_gen(dataset_type, train_generator, file_name, return_gen, params_d, coords=75):
    """Prepare dataset generator and optionally save to HDF5 file."""
...
def load_params_default(params_d):
    """Load default parameters and merge with provided parameters."""
...
def process_common(dataset, bm_train_type="cv", mv_fuse_data="none", 
    ...
    Args:
        dataset: Dataset name ('ntu' or 'pku')
        bm_train_type: Benchmark type ('cv', 'cs', 'all')
        mv_fuse_data: Multi-view fusion type ('none', 'train_only', 'both')
        method: ML method ('hmm', 'hcn', 'stgcn')
        method_type: Processing type ('classification', 'visualization', 'validation')
        params_d: Dictionary of parameters
    """
